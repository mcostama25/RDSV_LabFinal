...
chart_server="http://127.0.0.1:8080/"

for vnf in access cpe wan ctrl
do
  echo "[!] #### $vnf$NETNUM $chart_server$vnf$chart_suffix"
  helm -n $SDWNS install $vnf$NETNUM $chart_server$vnf$chart_suffix # /home/upm/Documents/RDSV/LAB_FINAL/helm-files/$vnf$chart_suffix
done
...
echo $VCPE
echo $VWAN
echo $VCTRL # añadido para la knf CTRL
